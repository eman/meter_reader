...
class GatewayError(Exception):
    ...
    def __init__(self, address: Tuple[str, int], command: str, error: str = '', code: int | None = None) -> None:
...
class EagleSocketClient(EagleClient):
    ...
    def __init__(self, address: str, port: int = DEFAULT_PORT, timeout: int = DEFAULT_TIMEOUT) -> None:
    ...
    def get_instantaneous_demand(self) -> InstantaneousDemand:
        ...
        data = self._run_command_dict(Name='get_instantaneous_demand', DeviceMacId=self.mac_id)
    ...
    def get_history_data(self, start_time: datetime | None = None, end_time: datetime | None = None, 
                        frequency: int = 0x384) -> List[CurrentSummation]:
        """
